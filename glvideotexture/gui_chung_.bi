...
Dim shared As Any Ptr guigfxbufferscreen
#If Not Defined(guinogfx)
  ScreenRes guiscreenx,guiscreeny,guidepth,2,fb.gfx_null'-1
  If guigfxbufferscreen<>0 Then ImageDestroy guigfxbufferscreen
  'guigfxbufferscreen=ImageCreate(guiscreenx,guiscreeny,RGB(0,200,0),guidepth)
  guigfxbufferscreen=ImageCreate(2048,2048,RGB(0,0,0),guidepth)
#EndIf 
Dim Shared As bitmapv4header guigfxinfo
With guigfxinfo
      .bV4Size = SizeOf(BITMAPV4HEADER)
      .bv4width=2048'guiscreenx
      .bv4height=-2048'guiscreeny 'negative value=>top to bottom bmp
...
return 0
End Function
Sub guiupdatescroll(ByVal hwin As HWND)
var xmin=0,xmax=10
getscrollrange(hwin,sb_ctl,@xmin,@xmax)
Var scrollpos=getscrollpos(hwin,sb_ctl)
Var request=LoWord(guiwparam)
Var scrollpos0=scrollpos
If request=SB_RIGHT Then scrollpos+=1
If request=SB_LEFT Then scrollpos-=1
If request=SB_LINERIGHT Then scrollpos+=1
If request=SB_LINELEFT Then scrollpos-=1
If request=SB_PAGERIGHT Then scrollpos+=max2(1,(xmax-xmin)/10)
If request=SB_PAGELEFT Then scrollpos-=max2(1,(xmax-xmin)/10)
If request=SB_THUMBPOSITION or request=SB_THUMBTRACK Then  
  scrollpos=HiWord(guiwparam)'(getScrollPos(getguih("win.text12"),sb_ctl))
EndIf
scrollpos=max2(xmin,min2(xmax,scrollpos))
If scrollpos<>scrollpos0 Then
	SetScrollPos(hwin,sb_ctl,scrollpos,TRUE)
EndIf
End Sub
      		...
      		If guigfxbuffer(i)<>0 Then
      			If guicontrolwind(i)=guiwindowname(iwindow) Then
      			  ...
      			  stretchDIBits(hDC, guicontrolx(i),guicontroly(i),guicontroldx(i),guicontroldy(i),_
       			  ...
       			  ScreenUnLock '/
       			  /'get (0,0)-(guicontroldx(i),guicontroldy(i)),guigfxbufferscreen
       			  Put (0,0),guigfxbuffer(i),pset
       			  ScreenLock
      			  stretchDIBits(hDC, guicontrolx(i),guicontroly(i),guicontroldx(i),guicontroldy(i),_
      			                0,0,guicontroldx(i),guicontroldy(i), _
                             ...
                             CPtr(bitmapinfo Ptr, @guigfxinfo), DIB_RGB_COLORS,SRCCOPY)
       			  ScreenUnLock       			  
       			  'Put (0,0),guigfxbufferscreen,(0,0)-(guicontroldx(i),guicontroldy(i)),PSet
      			  '/
       			  Put guigfxbufferscreen,(0,0),guigfxbuffer(i),PSet
                 'Line guigfxbufferscreen,(10,10)-(50,50),RGB(255,200,0),bf
                 'guigfxinfo.bv4width=guicontroldx(i)
                 'guigfxinfo.bv4height=-guicontroldy(i) 'negative value=>top to bottom bmp     			  
      			  stretchDIBits(hDC, guicontrolx(i),guicontroly(i),guicontroldx(i),guicontroldy(i),_
      			                0,0,guicontroldx(i),guicontroldy(i), _
                             guigfxbufferScreen+32,_
                             CPtr(bitmapinfo Ptr, @guigfxinfo), DIB_RGB_COLORS,SRCCOPY)
      			   '/
      			EndIf
      		EndIf
...
If guihicon>0 Then sendMessage(hWin,WM_SETICON,icon_big,Cast(LPARAM,guihIcon))
...
EndIf 
	...
	Case WM_HSCROLL,WM_VSCROLL
	   	'guinotice(getguihname(hwin),"wm_scroll")
      	'guinotice getguihname(lparam)
		   Var hwin2=Cast(hwnd,lparam)
		   iwindow=0
		   For i=1 To nguiwindow
		   	If hwin2=guihwindow(i) Then iwindow=i:Exit For
		   Next i
	      For i=1 To ngui
		     If guihcontrol(i)=hwin2 Then iwindow=i:Exit For
	      Next i
		   If iwindow>0 Then
		      If guicontrolsub(iwindow)<>0 Then
   	   	  	guihwin=hwin:guiLparam=lparam:guiwparam=wparam:guimsg=uMsg
   	   	  	guiupdatescroll(hwin2)
		      	guicontrolsub(iwindow)()
		      EndIf
		   EndIf
         Return DefWindowProc(hWin,uMsg,wParam,lParam)
         '
...
Dim As uint i
   ...
   Next
	...
	For i=1 To ngui
		...
		guihcontrol(i)=0
	Next
   If guigfxbufferscreen<>0 Then
   	ImageDestroy guigfxbufferscreen
   	guigfxbufferscreen=0
   EndIf
 	'PostQuitMessage(NULL)
...
guiaddcontrol(controlname,controltext,controlsub,_ 
	        controlx,controly,controldx,controldy,userflags)
End Sub 
Sub scrollbar(Byref controlname As String,ByRef verthoriz As String,ByVal controlsub As Sub(),_ 
	        ByVal controlx As uint,ByVal controly As uint,ByVal controldx As uint,ByVal controldy As uint,_
	        ByVal userflags As uint = 0)
guicontroltypeadd="scrollbar"
If verthoriz="vert" Then
	guicontrolstyleadd=SBS_VERT 'Or SBS_TOPALIGN
Else 
	guicontrolstyleadd=SBS_HORZ 'or SBS_BOTTOMALIGN
EndIf
guiaddcontrol(controlname,"scroll",controlsub,_ 
	        controlx,controly,controldx,controldy,userflags)
End Sub 
...
Sub guigetwindowpos(ByRef windowname As String,ByRef x As uint,ByRef y As uint,_ 
	                 ByRef dx As uint,ByRef dy As uint,ByVal tshow As Integer=1)
Dim As rect winrect
	getguih(windowname)
	If guih<>0 Then
	   If tshow=1 Then showwindow(guih,sw_shownoactivate)
...
End Sub 
Function guicreateDIBbmp(ByVal x As integer,ByVal y As integer) As hbitmap
...
End Function
#Ifndef noguijpeg
Function guiloadtexture(ByRef filename As String,ByVal maxcolor As uint=255, ByVal alph As uint=127,ByVal typealpha As integer=0) As uint 
	                     'bmp jpg gif ico
